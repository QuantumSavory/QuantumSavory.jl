...
This `firstgenrepeater_protocolzoo` is a much simpler implementation compared to the `firstgenrepeater`. Behind the scenes the simulation is basically the same, but this `v2` uses much more convenient higher-level abstractions using ProtocolZoo, so the user needs to write much less code.
...
```bash
julia --project=examples examples/firstgenrepeater_protocolzoo/1_entangler_example.jl
```

This produces a short animation (`firstgenrepeater_protocolzoo-01.entangler.mp4`) showing the entanglement links building up over time.
...
```bash
julia --project=examples examples/firstgenrepeater_protocolzoo/2_swapper_example.jl
```
...
```bash
julia --project=examples examples/firstgenrepeater_protocolzoo/3_purifier_example.jl
```
...
- [The "How To" doc page on setting up this simulation of a repeater chain](https://qs.quantumsavory.org/dev/howto/firstgenrepeater_protocolzoo/firstgenrepeater_protocolzoo)
