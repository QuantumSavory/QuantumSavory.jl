...
@resumable function GHZGraphSequentialProt(sim, n, net, link_success_prob, logging, rounds)
    ...
    while rounds != 0
        ...
        for i in 1:n
            @process entangle(sim, net, i, link_success_prob[i])
        end
...
function prepare_sim(n::Int, states_representation::AbstractRepresentation, noise_model::Union{AbstractBackground, Nothing}, link_success_prob::Vector{Float64}, seed::Int, logging::DataFrame, rounds::Int)
...
#@info df_all_runs
CSV.write(parsed_args["output_path"]*"qs_sequential$(n)_heterog.csv", df_all_runs)