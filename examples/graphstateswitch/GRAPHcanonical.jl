...
include("utils.jl")
##
@resumable function GraphCanonicalProt(sim, n, net, link_success_prob, logging, rounds)
        ...
        while true
            ...
            if length(active_clients) == n
                for e in edges(graph)
                    apply!((net[1][src(e)], net[1][dst(e)]), ZCZ; time=now(sim))
                end
                for i in 1:n
...
function prepare_sim(n::Int, states_representation::AbstractRepresentation, noise_model::Union{AbstractBackground, Nothing}, link_success_prob::Float64, seed::Int, logging::DataFrame, rounds::Int)
    ...
    @process GraphCanonicalProt(sim, n, net, link_success_prob, logging, rounds)
    return sim
end
...
for n in [4]
    ...
    #@info df_all_runs
    CSV.write("examples/graphstateswitch/output/factory/qs_graph4canonical$(n).csv", df_all_runs)
end