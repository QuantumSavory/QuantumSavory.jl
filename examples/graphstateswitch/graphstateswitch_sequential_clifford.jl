...
@resumable function entangle(sim, net, client, link_success_prob)
    ...
    entangler = EntanglerProt(
        ...
        success_prob=link_success_prob, rounds=1, attempts=-1, attempt_time=1.0, pairstate=SProjector(StabilizerState("XX ZZ")) # Note: generate a two-graph state instead of a bell pair
        )
...
function prepare_sim(path_to_graph_data, link_success_prob, seed, logging, rounds)
    ...
    switch = Register(fill(Qubit(),2*n), fill(CliffordRepr(),2*n)) # storage and communication qubits at the switch
    clients = Register(n, CliffordRepr()) # client qubits
...
for link_success_prob in range(0.1,1,10)
    ...
    sim = prepare_sim("examples/graphstateswitch/input/7_20250127.pickle", link_success_prob, seed, logging, rounds)
...
@info all_runs
#CSV.write("examples/graphstateswitch/output/sequential_clifford.csv", all_runs)