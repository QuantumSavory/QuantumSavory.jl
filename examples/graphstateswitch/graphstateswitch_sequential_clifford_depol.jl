...
@resumable function PiecemakerProt(sim, n, net, graphdata, operationdata, link_success_prob, logging, rounds)
    ...
    b = net[2] # clients
    ε = 1e-12
    graph = Graph() # graph to be used for teleportation
...
function prepare_sim(τ, n, graphdata, operationdata, link_success_prob, seed, logging, rounds)
    
    # Set a random seed
    Random.seed!(seed)
    
    switch = Register(fill(Qubit(), 2*n), fill(CliffordRepr(), 2*n), fill(Depolarization(τ), 2*n)) # storage and communication qubits at the switch # fill(T2Dephasing(1.0), 2*n)
    clients = Register(fill(Qubit(), n),  fill(CliffordRepr(), n), fill(Depolarization(τ), n)) # client qubits
...
for nr in [2, 4, 7, 8, 9, 18, 40, 100] # Graph identifier 
    for τ in [10.0^i for i in 0:3]
        ...
        for (f, link_success_prob) in enumerate(range(0.01,1,10))
            ...
            sim = prepare_sim(τ, n, graphdata, operationdata, link_success_prob, seed, logging, rounds)
        ...
        # @info all_runs
        CSV.write("examples/graphstateswitch/output/sequential_clifford_noisy_nr$(nr)_τ$(τ).csv", all_runs)
    end
end