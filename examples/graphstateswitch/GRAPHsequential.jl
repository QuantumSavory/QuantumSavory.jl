...
include("utils.jl")
##
@resumable function GraphSequentialProt(sim, n, net, link_success_prob, logging, rounds)
    ...
    while rounds != 0
        ...
        while true # until all clients are measured out
            ...
            if (counter_clients == n) && (length(active_clients) == 2) # measuring out core slots (first one to arrive) at last
                apply!((net[1][active_clients[1]], net[1][active_clients[2]]), ZCZ; time=now(sim))
                push!(CZs, (active_clients[1], active_clients[2]))
                for core_client in active_clients
                    zmeas = project_traceout!(net[1][core_client], σˣ) 
                    if zmeas==2 apply!(net[2][core_client], Z) end
                end
                break
            end
            ...
            i = 1
            if counter_clients > 2
                while length(active_clients) > 2
                    idx = pop!(active_clients)
                    apply!((net[1][active_clients[i]], net[1][idx]), ZCZ; time=now(sim))
                    push!(CZs, (active_clients[i], idx))
                    i += 1
        ...
        obs = projector(StabilizerState(Stabilizer(graph)))
...
function prepare_sim(n::Int, states_representation::AbstractRepresentation, noise_model::Union{AbstractBackground, Nothing}, link_success_prob::Float64, seed::Int, logging::DataFrame, rounds::Int)
    ...
    @process GraphSequentialProt(sim, n, net, link_success_prob, logging, rounds)
    return sim
end
...
for n in [4]
    ...
    #@info df_all_runs
    CSV.write("examples/graphstateswitch/output/factory/qs_graph4sequential$(n).csv", df_all_runs)
end