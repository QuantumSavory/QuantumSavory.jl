...
for i in 1:10
    ...
    @test collect(values(net[1].tag_info))[1][1] == Tag(EntanglementCounterpart, 2, 1)
    ...
    @test collect(values(net[1].tag_info))[1][1] == Tag(EntanglementCounterpart, 2, 1)
    @test [t[1] for t in collect(values(net[2].tag_info))] == [Tag(EntanglementCounterpart, 1, 1), Tag(EntanglementCounterpart, 3, 1)]
    @test [t[1] for t in collect(values(net[3].tag_info))] == [Tag(EntanglementCounterpart, 2, 2), Tag(EntanglementCounterpart, 4, 1)]
    @test [t[1] for t in collect(values(net[4].tag_info))] == [Tag(EntanglementCounterpart, 3, 2)]
    ...
    @test collect(values(net[1].tag_info))[1][1] == Tag(EntanglementCounterpart, 2, 1)
    @test [t[1] for t in collect(values(net[2].tag_info))] == [Tag(EntanglementHistory, 1, 1, 3, 1, 2),Tag(EntanglementHistory, 3, 1, 1, 1, 1)]
    @test [t[1] for t in collect(values(net[3].tag_info))] == [Tag(EntanglementHistory, 4, 1, 2, 2, 1), Tag(EntanglementHistory, 2, 2, 4, 1, 2)]
    @test [t[1] for t in collect(values(net[4].tag_info))] == [Tag(EntanglementCounterpart, 3, 2)]
