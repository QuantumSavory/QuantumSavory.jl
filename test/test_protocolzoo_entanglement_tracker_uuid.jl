@testitem "ProtocolZoo Entanglement Tracker UUID" tags=[:protocolzoo_entanglement_tracker_uuid] begin
using Revise
using ResumableFunctions
using ConcurrentSim
using QuantumSavory.ProtocolZoo
using QuantumSavory.ProtocolZoo: EntanglementUUID, EntanglementUpdateUUID, EntanglementDeleteUUID, generate_pair_uuid
using Graphs
...
uuid_tags_1 = [tag for tag in (net[1].tag_info[i].tag for i in net[1].guids) if tag.type == EntanglementUUID]
uuid_tags_2 = [tag for tag in (net[2].tag_info[i].tag for i in net[2].guids) if tag.type == EntanglementUUID]
...
swapper2 = SwapperProtUUID(sim, net, 2; nodeL = <(2), nodeH = >(2), chooseL = argmin, chooseH = argmax, rounds = 1)
@process swapper2()
run(sim, 50)
...
uuid_tags_3 = [tag for tag in (net[3].tag_info[i].tag for i in net[3].guids) if tag.type == EntanglementUUID]
