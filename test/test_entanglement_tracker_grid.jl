...
for path in paths
    ...
    @test [t[1] for t in collect(values(net[1].tag_info))] == [Tag(EntanglementCounterpart, path[1], 1)]
    ...
    @test [t[1] for t in collect(values(net[1].tag_info))] == [Tag(EntanglementCounterpart, path[1], 1)]
    @test [t[1] for t in collect(values(net[path[1]].tag_info))] == [Tag(EntanglementCounterpart, 1, 1), Tag(EntanglementCounterpart, path[2], 1)]
    @test [t[1] for t in collect(values(net[path[2]].tag_info))] == [Tag(EntanglementCounterpart, path[3], 1), Tag(EntanglementCounterpart, path[1], 2)]
    @test [t[1] for t in collect(values(net[path[3]].tag_info))] == [Tag(EntanglementCounterpart, path[2], 2), Tag(EntanglementCounterpart, path[4], 1)]
    @test [t[1] for t in collect(values(net[path[4]].tag_info))] == [Tag(EntanglementCounterpart, path[5], 1), Tag(EntanglementCounterpart, path[3], 2)]
    @test [t[1] for t in collect(values(net[path[5]].tag_info))] == [Tag(EntanglementCounterpart, 16, 1), Tag(EntanglementCounterpart, path[4], 2)]
    @test [t[1] for t in collect(values(net[16].tag_info))] == [Tag(EntanglementCounterpart, path[5], 2)]
    ...
    @test [t[1] for t in collect(values(net[1].tag_info))] == [Tag(EntanglementCounterpart, path[1], 1)]
    @test [t[1] for t in collect(values(net[path[1]].tag_info))] == [Tag(EntanglementHistory, 1, 1, path[2], 1, 2), Tag(EntanglementHistory, path[2], 1, 1, 1, 1)]
    @test [t[1] for t in collect(values(net[path[2]].tag_info))] == [Tag(EntanglementHistory, path[1], 2, path[3], 1, 2), Tag(EntanglementHistory, path[3], 1, path[1], 2, 1)]
    @test [t[1] for t in collect(values(net[path[3]].tag_info))] == [Tag(EntanglementHistory, path[4], 1, path[2], 2, 1), Tag(EntanglementHistory, path[2], 2, path[4], 1, 2)]
    @test [t[1] for t in collect(values(net[path[4]].tag_info))] == [Tag(EntanglementHistory, path[5], 1, path[3], 2, 1), Tag(EntanglementHistory, path[3], 2, path[5], 1, 2)]
    @test [t[1] for t in collect(values(net[path[5]].tag_info))] == [Tag(EntanglementHistory, 16, 1, path[4], 2, 1), Tag(EntanglementHistory, path[4], 2, 16, 1, 2)]
    @test [t[1] for t in collect(values(net[16].tag_info))] == [Tag(EntanglementCounterpart, path[5], 2)]
