...
# Test EntanglerProtUUID shorthand constructor
eprot_uuid = EntanglerProtUUID(net, 1, 2; success_prob=0.8, rounds=2)
@test eprot_uuid.sim === get_time_tracker(net)

# Test EntanglementTracker shorthand constructor
etracker = EntanglementTracker(net, 2)
@test etracker.sim === get_time_tracker(net)

# Test EntanglementTrackerUUID shorthand constructor
etracker_uuid = EntanglementTrackerUUID(net, 2)
@test etracker_uuid.sim === get_time_tracker(net)
...
# Test EntanglementConsumerUUID shorthand constructor
econsumer_uuid = EntanglementConsumerUUID(net, 1, 3; period=0.2)
@test econsumer_uuid.sim === get_time_tracker(net)
...
# Test SwapperProtUUID shorthand constructor
swapper_uuid = SwapperProtUUID(net, 2; rounds=5, local_busy_time=0.1)
@test swapper_uuid.sim === get_time_tracker(net)
...
# Test CutoffProtUUID shorthand constructor
cutoff_uuid = CutoffProtUUID(net, 3; period=0.05, retention_time=10.0)
@test cutoff_uuid.sim === get_time_tracker(net)
