...
struct SuperdenseTestPair
    remote_node::Int
    remote_slot::Int
end
QuantumSavory.Tag(tag::SuperdenseTestPair) = Tag(SuperdenseTestPair, tag.remote_node, tag.remote_slot)

function add_superdense_pair!(net, slot; tagtype=EntanglementCounterpart)
    initialize!((net[1, slot], net[2, slot]), StabilizerState("ZZ XX"); time=now(get_time_tracker(net)))
    tag!(net[1, slot], tagtype, 2, slot)
    tag!(net[2, slot], tagtype, 1, slot)
end
...
@testset "SuperdenseCodingProt" begin
    ...
    @testset "delivers with Clifford-backed registers" begin
        net = RegisterNet([Register(4, CliffordRepr()), Register(5, CliffordRepr())]; quantum_delay=0.25)
        sim = get_time_tracker(net)
        for slot in 1:4
            add_superdense_pair!(net, slot)
        end
        ...
        for (uuid, bits) in enumerate(payloads)
            delivery = take_delivery!(net, bits, uuid)
            @test !isnothing(delivery)
            @test delivery.tag[4] isa Int
            @test delivery.tag[5] isa Int
        end
        @test length(prot._log) == 4
        @test all(entry.bits isa NTuple{2,Int} for entry in prot._log)
    end

    @testset "uses entanglement generated by EntanglerProt" begin
        ...
        @process entangler()
        @process prot()
        put!(net[1], SuperdenseMessage(1, 2, (1, 0), 23))

        run(sim, 2.0)

        delivery = take_delivery!(net, (1, 0), 23)
        @test !isnothing(delivery)
        @test only(prot._log).send_slot == 1
        @test only(prot._log).entangled_slot == 1
    end

    @testset "supports custom pair tags and randomized receive-slot selection" begin
        net = RegisterNet([Register(1), Register(3)]; quantum_delay=0.1)
        sim = get_time_tracker(net)
        add_superdense_pair!(net, 1; tagtype=SuperdenseTestPair)

        prot = SuperdenseCodingProt(net, 1, 2; tag=SuperdenseTestPair, randomize=true, period=0.05)
        @process prot()
        put!(net[1], SuperdenseMessage(1, 2, (0, 1), 24))

        run(sim, 2.0)

        delivery = take_delivery!(net, (0, 1), 24)
        @test !isnothing(delivery)
        @test only(prot._log).receive_slot in (2, 3)
    end
    ...
    @testset "releases locks when guarded helper paths throw" begin
        net = RegisterNet([Register(2), Register(4)])
        initialize!(net[1, 1])

        send_slot = net[1, 1]
        entangled_slot = net[2, 1]
        receive_slot = net[2, 2]
        qchannel_resource = net[2, 3]
        for resource in (send_slot, entangled_slot, receive_slot, qchannel_resource)
            lock(resource)
        end
        err = try
            QuantumSavory.ProtocolZoo._send_superdense_qubit!(
                send_slot,
                (0, 0),
                nothing,
                entangled_slot,
        ...
        @test all(resource -> !islocked(resource), (send_slot, entangled_slot, receive_slot, qchannel_resource))

        prot = SuperdenseCodingProt(net, 1, 2)
        send_slot = net[1, 2]
        entangled_slot = net[2, 1]
        receive_slot = net[2, 2]
        qchannel_resource = net[2, 4]
        initialize!(entangled_slot)
        for resource in (send_slot, entangled_slot, receive_slot, qchannel_resource)
            lock(resource)
        end
        err = try
            QuantumSavory.ProtocolZoo._finish_superdense_delivery!(
                prot,
                net[2],
                send_slot,
                entangled_slot,
