...
@testset "QTCP LinkController external inventory" begin
    ...
    @testset "tag invalidation retries without leaking locks" begin
        ...
        invalidated_state = QuantumSavory.stateof(net[1][2])
        ...
        isnothing(reply) || @test reply.tag[4] == 1
        @test query(
            net[1][2], invalidated_a.tag; assigned=true
        ).id == invalidated_a.id
        @test isnothing(query(
            net[2][2], invalidated_b.tag; assigned=true
        ))
        @test QuantumSavory.stateof(net[1][2]) === invalidated_state ===
            QuantumSavory.stateof(net[2][2])
        @test all(!islocked(net[node][slot]) for node in 1:2 for slot in 1:3)
    end
