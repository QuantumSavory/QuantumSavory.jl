...
- Document `qchannel` / `QuantumChannel` with a dedicated explanations page and a `put!`/`take!` tutorial (#509).
- `qchannel` now errors with a clear message when the requested pair is not a direct graph edge.
